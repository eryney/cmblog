...
Then: /tmp/state_plot_venv/bin/python may_2026/state_plot_codex_corrected.py
Produces: may_2026/codex_corrected_figure.png
"""
...
OUT_NAME    = "codex_corrected_figure"
...
def main():
    ...
    fig.suptitle(
        "What STATE predicts for melanoma vs. colorectal cancer transcriptomes\n"
        "Dabrafenib (BRAF inhibitor) vs. Erlotinib (EGFR inhibitor)",
    ...
    ax2.set_ylabel("$\\Delta$ log1p expression\n(melanoma cells)", fontsize=10, labelpad=8)
    ...
    if r_cross_mel > 0.8:
        cross_str = (
            ...
            f"High correlation suggests cell-type context\n"
            f"dominates drug mechanism in this setup"
        )
        ax3.text(
            0.03, 0.60, cross_str,
            transform=ax3.transAxes,
    ...
    ax3.set_xlabel("$\\Delta$ log1p expression, colorectal", fontsize=10, labelpad=8)
    ax3.set_ylabel("$\\Delta$ log1p expression, melanoma",    fontsize=10, labelpad=8)
    ...
    caption = (
        ...
        f"cross-drug $R = {r_cross_mel:.3f}$ suggests STATE is dominated by "
        f"cell-type context rather than drug-specific mechanism in this setup. "
        f"Cells were selected by disease and tissue from CellxGene Census, not by BRAF genotype."
    )
    plt.figtext(
        0.5, 0.035, caption,
    ...
    plt.tight_layout(pad=2.8, rect=[0, 0.13, 1, 0.90])
